...
export default function PostToc(props: { toc: Array<{ text: string; level: number }> }) {
  ...
  return (
    <div className="fixed text-sm z-99 top-100px right-20 hidden xl-block list-none">
          ...
          toc && toc.map((item, index) => {
            if (item.level === 1) {
              return (
                  ...
                  <a className="no-underline text-16px op-60 hover-op-100" href={`#${item.text}`} >{item.text}</a>
                  </li>
              )
            }
